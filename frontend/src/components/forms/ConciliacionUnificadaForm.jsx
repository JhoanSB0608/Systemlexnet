...
const ConciliacionUnificadaForm = ({ onSubmit, resetToken, initialData, isUpdating }) => {
  const theme = useTheme();
  ...
  const tipoSolicitud = TIPO_CONCILIACION;
  ...
  useEffect(() => {
    ...
    if (autosaveEnabled && borradorId) {
      (async () => {
        try {
          console.log('[ConciliacionUnificadaForm] Reanudando borrador:', borradorId);
          ...
          console.error('[ConciliacionUnificadaForm] No se pudo reanudar el borrador:', error);
  ...
  const customOnSubmit = async (data) => { // Make it async
    ...
    if (autosaveEnabled) {
      try {
        ...
        console.warn('[ConciliacionUnificadaForm] No se pudo vaciar la cola de auto-guardado:', e);
      }
    }
    ...
    await onSubmit(dataToSend); // Pass the processed data object, not FormData
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[ConciliacionUnificadaForm] Error al encolar auto-guardado:', e);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [debouncedWatchedAll]);
